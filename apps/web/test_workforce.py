...
class WorkforceTests(TestCase):
    ...
    def test_registration_inactive_and_csrf_enforced(self):
        ...
        self.assertEqual(dashboard.context['header_notifications_count'], 1)
        self.assertTrue(any(item['url'] == '/employees/?status=inactive' and 'newstaff' in item['title'] for item in dashboard.context['header_notifications']))
        self.assertContains(self.client.get('/employees/?status=inactive'), 'newstaff')
        self.assertNotContains(self.client.get('/employees/?status=inactive'), 'Private employee')
        self.login(self.caller)
        self.assertNotContains(self.client.get('/'), 'newstaff')
        self.assertEqual(self.client.get(f'/employees/{new.pk}/').status_code, 403)
        self.login(self.admin)
        response = self.client.post(f'/employees/{new.pk}/', {'role': 'IT', 'is_active': 'on'})
        self.assertEqual(response.status_code, 302)
        new.refresh_from_db()
        self.assertTrue(new.is_active)
        self.assertEqual(new.role, 'IT')
        self.assertEqual(self.client.get('/').context['header_notifications_count'], 0)
        self.assertTrue(self.client.login(username='newstaff', password='Secure-test-pass-912'))
